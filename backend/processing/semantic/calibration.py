import math


class BayesianCalibrator:
    """
    Calibration bayésienne simple et robuste
    """

    def __init__(self, alpha: float = 2.0, beta: float = 2.0):
        self.alpha = alpha
        self.beta = beta

    def calibrate(self, similarity_score: float) -> float:
        """
        Transforme un score cosinus en probabilité calibrée
        """
        # Clamp
        s = max(0.0, min(1.0, similarity_score))

        # Bayesian smoothing
        return (s * self.alpha) / (s * self.alpha + (1 - s) * self.beta)


# processing/semantic/classifier.py

from typing import Any, Dict, List

from processing.semantic.calibration import BayesianCalibrator
from processing.semantic.embeddings import EmbeddingService
from processing.semantic.normalization import extract_semantic_fields


class SemanticIndicatorClassifier:
    def __init__(
        self,
        vectorstore,
        batch_size: int = 16,
        confidence_threshold: float = 0.85,
    ):
        self.vectorstore = vectorstore
        self.batch_size = batch_size
        self.embeddings = EmbeddingService()
        self.calibrator = BayesianCalibrator()
        self.threshold = confidence_threshold

    def _batch(self, items: List[str]):
        for i in range(0, len(items), self.batch_size):
            yield items[i : i + self.batch_size]

    def classify_row(self, row: Dict[str, Any]) -> List[Dict[str, Any]]:
        semantic_fields = extract_semantic_fields(row)
        if not semantic_fields:
            return []

        results: List[Dict[str, Any]] = []

        for batch in self._batch(list(semantic_fields.keys())):
            vectors = self.embeddings.embed_many(batch)

            for field, vector in zip(batch, vectors):
                matches = self.vectorstore.similarity_search_with_score_by_vector(
                    vector, k=3
                )

                candidates = []
                for doc, score in matches:
                    prob = self.calibrator.calibrate(score)

                    candidates.append(
                        {
                            "indicator_id": doc.metadata.get("indicator_id"),
                            "indicator_code": doc.metadata.get("indicator_code"),
                            "category": doc.metadata.get("category_code"),
                            "unit": doc.metadata.get("unit"),
                            "similarity": round(score, 3),
                            "confidence": round(prob, 3),
                            "auto_validated": prob >= self.threshold,
                        }
                    )

                results.append(
                    {
                        "raw_field": field,
                        "raw_value": semantic_fields[field],
                        "candidates": candidates,
                    }
                )

        return results
